...
const {createTeamIfNotExists, upsertUserAccessToken, subscribe, paddleSubscribe} = require('./util');
  ...
  try {
    ...
    const requestParams = {
      ...
      redirect_uri: selected
        ? `https://paddle.smithmayowa20.now.sh/oauth?selected=${selected}`
        : "https://paddle.smithmayowa20.now.sh/oauth"
    };
    ...
    if (selected === "plan_GWEUqaZ2fT6emr") {
      console.log("Personal, so subscribing now")
      await paddleSubscribe({
        client,
    ...
    if (selected) {
      redirect(res, 302, `/?selected=${selected}&slack_access_token=${slack_access_token}`);
    } else {
      redirect(res, 302, "/");
    }
  }
